...
public readonly partial struct Int5T :
    IConvertible,
    IBinaryInteger<Int5T>,
    ISignedNumber<Int5T>,
    ITernaryInteger<Int5T>
{
...
public static implicit operator Int64(Int5T value) => (Int64)value.value;
    ...
    /// <summary>
    /// Adds two <see cref="Int5T"/> values and returns the result.
    /// </summary>
    ...
    /// <summary>
    /// Subtracts one <see cref="Int5T"/> value from another and returns the result.
    /// </summary>
    ...
    /// <summary>
    /// Multiplies two <see cref="Int5T"/> values and returns the result.
    /// </summary>
    ...
    /// <summary>
    /// Divides one <see cref="Int5T"/> value by another and returns the result.
    /// </summary>
    ...
    /// <summary>
    /// Returns the remainder that results from dividing one <see cref="Int5T"/> value by another.
    /// </summary>
    /// <param name="left">The value to be divided (the dividend).</param>
    /// <param name="right">The value to divide by (the divisor).</param>
    ...
    /// <summary>
    /// Negates the specified <see cref="Int5T"/> value.
    /// </summary>
    ...
    /// <summary>
    /// Returns the specified <see cref="Int5T"/> value; the sign of the value is unchanged.
    /// </summary>
    ...
    public static Int5T RotateLeft(Int5T value, int rotateAmount) =>
    ...
    static bool IBinaryNumber<Int5T>.IsPow2(Int5T value) =>
        value.value > 0 && (value.value & (value.value - 1)) == 0;

    static Int5T IBinaryNumber<Int5T>.Log2(Int5T value) =>
        Create(BitOperations.Log2((uint)value.value));

    int IBinaryInteger<Int5T>.GetByteCount() => sizeof(SByte);

    int IBinaryInteger<Int5T>.GetShortestBitLength() =>
        value == 0 ? 1 : BitOperations.Log2((uint)Math.Abs(value)) + 1;

    static Int5T IBinaryInteger<Int5T>.PopCount(Int5T value) =>
        Create(BitOperations.PopCount((uint)value.value));

    static Int5T IBinaryInteger<Int5T>.TrailingZeroCount(Int5T value) =>
        Create(BitOperations.TrailingZeroCount((uint)value.value));

    bool IBinaryInteger<Int5T>.TryWriteBigEndian(Span<byte> destination, out int bytesWritten)
    {
    ...
    bool IBinaryInteger<Int5T>.TryWriteLittleEndian(Span<byte> destination, out int bytesWritten)
    {
        if (destination.Length < sizeof(SByte))
        {
            bytesWritten = 0;
    ...
    static bool IBinaryInteger<Int5T>.TryReadBigEndian(ReadOnlySpan<byte> source, bool isUnsigned, out Int5T result)
    {
    ...
    static bool IBinaryInteger<Int5T>.TryReadLittleEndian(ReadOnlySpan<byte> source, bool isUnsigned, out Int5T result)
    {
        if (source.Length < sizeof(SByte))
        {
            result = default;
    ...
    static Int5T IBitwiseOperators<Int5T, Int5T, Int5T>.operator &(Int5T left, Int5T right) =>
    ...
    static Int5T IBitwiseOperators<Int5T, Int5T, Int5T>.operator ~(Int5T value) =>
        Create(~value.value);

    static Int5T IShiftOperators<Int5T, int, Int5T>.operator <<(Int5T value, int shiftAmount) =>
